...
<div class="subject {bool2noprint(subject.global.all)} {bool2noprint(subject.global.p_all)}" > <!--class noprint if deselected-->
	...
	{#if typeof subject.data !== "undefined"}
		...
		{#each Object.entries(subject.data) || [] as [week, value]}
				<div style="border-top-color: black; background-color: {range.weeks[week].plan.color} !important" aria-hidden=true on:mouseenter={()=>{value=change(value, 'all', week)}} class="square week" >{range.infcount(week)}</div>
		{/each}
		</div>
	{/if}
	{#if layer==0}
