...
// @ts-nocheck
	...
	function dateColor(week){
		if (typeof range.weeks[week].entryname !== 'undefined'){
			return 'red';
		}
		else{
			return 'white';
...
function calc(subject) {
	...
	onMount(() => 
	{
		...
		if (!subject.children.length){
			subject.global.p_base = parseFloat(subject.subject.price_base);
		}
		else{
		...
		subject.global.end_self = range.max(subject.data, 'b_self')+4; //4 extrawochen im lernsystem
...
<div class="subject {bool2noprint(subject.global.all)} {bool2noprint(subject.global.p_all)}" > <!--class noprint if deselected-->
	<div class="public-row title layer{layer}">
		...
		<div class="sum-lesson sum">
			{#if !(subject.global.n_visit>0 && subject.global.visit)}
				...
				{:else}
				<div class="sumpart {bool2noprint(subject.global.n_watch)}"></div>
					<div class="sumpart {bool2noprint(subject.global.n_watch)}">{subject.global.n_watch} <img class="icon" src={watch} alt=""/> </div>
				{/if}
			{:else}
				<div class="sumpart {bool2noprint(subject.global.n_visit)}">{subject.global.n_visit}<img class="icon" src={visit} alt=""/> </div>
				<div class="sumpart {bool2noprint(subject.global.n_watch)}">{subject.global.n_watch} <img class="icon" src={watch} alt=""/> </div>
			{/if}
		</div>
	...
	{#if typeof subject.data !== "undefined"}
		...
		{#each Object.entries(subject.data) || [] as [week, value]}
				<div style="border-style: solid; border-color: {dateColor(week)}; background-color: {range.weeks[week].plan.color} !important" aria-hidden=true on:mouseenter={()=>{value=change(value, 'all', week)}} class="square week" >{range.infcount(week)}</div>
		{/each}
		</div>
	{/if}
	{#if layer==0}
	...
	{#if range.show && subject.global.all}
		...
		<div class="public-row {bool2noprint(subject.global.visit && parent_global.visit && layer)}" aria-hidden=true>
	...
	{#if  parent_global.watch}
		...
		<div class="public-row {bool2noprint(subject.global.watch && layer)}">
		...
		<div class="square-row no-print">
			{#each Object.entries(subject.data) || [] as [week, value]}
				{#if value.c_visit}
					<div aria-hidden=true class="square {bool2active(value.b_watch)} {bool2parentactive(value.parent_watch)}" on:mouseenter={()=>{value=change(value, 'watch', week)}}>{value.n_visit}</div>
				{:else}
					<div aria-hidden=true class="square idle" >{value.n_visit}</div>
				{/if}
			{/each}
		</div>
		{/if}
	...
	{#if  parent_global.self}
		...
		<br class="{bool2noprint(subject.global.self && layer)}"/>
			<div class="public-row {bool2noprint(subject.global.self && layer)}">
				<div class="name sum pad{layer}">
				...
				<div class="sum-lesson sum"> 
						...
						<span class="no-print">Grundgebühr: {subject.global.p_base}</span>
				</div>
...
<br/>
<span class="no-print">{@html range.invoice_text}</span>
{/if}
