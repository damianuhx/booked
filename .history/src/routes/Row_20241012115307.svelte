...
{#if layer==0 && range.show}


<!--Duplicate from above for print version-->
<div class="hr" style=""></div>

{#if range.show && subject.global.all}
		{#if  parent_global.visit}
		<br class="{bool2noprint(subject.global.visit)}"/>
		<div class="public-row title {bool2noprint(subject.global.visit && parent_global.visit)} {bool2greyed(subject.global.visit)}" aria-hidden=true>
			<div class="name sum pad1">
				<input class="no-print checkbox" type="checkbox" bind:checked={subject.global.visit}>
				Schulbesuch
			</div>
			<div class="sum-lesson sum">{subject.global.n_visit} Lektionen </div>
	...
	{#if  parent_global.watch}
		...
		<div class="public-row title {bool2noprint(subject.global.watch)} {bool2greyed(subject.global.watch)}">
	...
	{#if  parent_global.self}
		...
		<br class="{bool2noprint(subject.global.self)}"/>
			<div class="public-row title {bool2noprint(subject.global.self)} {bool2greyed(subject.global.self)}">
				<div class="name sum pad1">
					<input class="no-print checkbox" type="checkbox" bind:checked={subject.global.self}>
					Lernportal
				</div>
